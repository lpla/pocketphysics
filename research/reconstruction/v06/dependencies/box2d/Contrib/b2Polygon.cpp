...
#line 173 "Contrib/b2Polygon.cpp"
		b2Assert(currIndex < newNVertices);
#line 174 "Contrib/b2Polygon.cpp"
		newx[currIndex] = x[i];
		newy[currIndex] = y[i];
		++currIndex;
	}
...
bool b2Polygon::IsUsable(bool printErrors){
	...
	for (int32 i=0; i<nVertices; ++i){
		...
		b2Vec2 d;
		d.x = b2Dot(n1, v) - COLLAPSE_DIST_SQR;
		d.y = b2Dot(n2, v) - COLLAPSE_DIST_SQR;
...
#line 512 "Contrib/b2Polygon.cpp"
	b2Assert(nVertices <= b2_maxPolygonVertices);
#line 517 "Contrib/b2Polygon.cpp"
//	printf("Adding...\n");
	int32 offset = 0;
    for (int32 i = 0; i < nVertices; ++i) {
...
//			ReversePolygon(p->x, p->y, p->nVertices); //reset orientation
     ...
     */
b2PolygonDef* DecomposeConvexAndAddTo(b2World*, b2Polygon* p, b2Body* bd,
...
//		printf("npolys: %d",nPolys);
		...
		int32 extra = 0;
			...
			if (curr.nVertices == 3) {
				for (int j = 0; j < 3; ++j) {
					lower = (j == 0) ? (curr.nVertices - 1) : (j - 1);
					middle = j;
					upper = (j == curr.nVertices - 1) ? 0 : (j + 1);
					...
					float32 norm0 = sqrtf(dx0 * dx0 + dy0 * dy0);
					float32 norm1 = sqrtf(dx1 * dx1 + dy1 * dy1);
					if (!(norm0 > 0.0f && norm1 > 0.0f)) goto Skip;
					dx0 /= norm0;
					dy0 /= norm0;
					dx1 /= norm1;
					dy1 /= norm1;
...
#line 1106 "Contrib/b2Polygon.cpp"
		b2Assert(nVert > 2);
#line 1114 "Contrib/b2Polygon.cpp"
...
#define PP_COMPILER_STATE_ASSERT(expression, label) \
	((expression) ? (void)0 : __assert(__FILE__, __LINE__, label))

b2Polygon TraceEdgeCompilerState(b2Polygon* p){
			...
			for (int32 j=i+1; j < nNodes; ++j){
				...
				if (diff.LengthSquared() <= COLLAPSE_DIST_SQR){
					...
					for (int32 k=0; k < njConn; ++k){
						...
						PP_COMPILER_STATE_ASSERT(knode != jnode, "jnode->nConnected == 0");
						if (knode != inode) {
	...
	PP_COMPILER_STATE_ASSERT(currentNode->nConnected > 0, "false");
...
#undef PP_COMPILER_STATE_ASSERT
...
#line 1527 "Contrib/b2Polygon.cpp"
