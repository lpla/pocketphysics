...
UL_IMAGE *ulLoadImagePNG(VIRTUAL_FILE *f, int location, int pixelFormat)
{
	...
	int wantedPixelFormat = pixelFormat, transparentColor = -1;
...
/*	if (depth > 8 && color_type == PNG_COLOR_TYPE_PALETTE)			{
	...
	if (pixelFormat == UL_PF_PAL5_A3 || pixelFormat == UL_PF_PAL3_A5)		{
		alphaBuffer = malloc(width * height * 4);
		convertAlpha = 1;
	}

	//Pas de palette dans le PNG mais notre pixelformat en veut une? On le charge en 16 bits et on fera la conversion après
	if (!pPngInfo->num_palette && ul_pixelWidth[pixelFormat] <= 8)
		pixelFormat = UL_PF_5551;
			...
			if (ul_pixelWidth[pixelFormat] <= 8)				{
				...
				if (img->palette)			{
				   ...
				   pPngInfo->num_palette = ulMin(pPngInfo->num_palette, img->palCount);
					...
					//Toujours pas trouvé de couleur transparente??? - on décale tout d'un alors
...
/*					//Check avec la couleur transparente
					...
					if (convertAlpha)			{
						...
						alphaBuffer[offset] = r;
						alphaBuffer[offset + 1] = g;
						alphaBuffer[offset + 2] = b;
						alphaBuffer[offset + 3] = a;
					}
					else if (pixelFormat == UL_PF_5551)
	...
	if (convertAlpha)		{
		img = ulConvertImageToPalettedAlpha(img, alphaBuffer, UL_IN_RAM, wantedPixelFormat);
		free(alphaBuffer);
	}
	else if (wantedPixelFormat != pixelFormat)
